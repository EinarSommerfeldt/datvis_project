...
def convert_annotations_to_YOLO(rdd2022folder, folderpath):
    filelist = []
    ...
    if not os.path.exists(rdd2022folder+"/labels"):
            os.makedirs(rdd2022folder+"/labels")
...
for d in dirs:
    path = prefix + d
    convert_annotations_to_YOLO(prefix, path)
